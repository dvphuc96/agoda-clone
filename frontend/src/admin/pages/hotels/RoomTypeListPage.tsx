      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        ...
        <aside className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <form className="mt-4 space-y-3" onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>
            {(['name', 'bed_type'] as const).map((field) => <input key={field} value={String(form[field] ?? '')} onChange={(event) => setForm({ ...form, [field]: event.target.value })} placeholder={field} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />)}
            <div className="grid grid-cols-3 gap-2">
              <input type="number" value={form.max_guests} onChange={(event) => setForm({ ...form, max_guests: Number(event.target.value) })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
              <input type="number" value={String(form.price_per_night)} onChange={(event) => setForm({ ...form, price_per_night: event.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
              <input type="number" value={form.total_rooms} onChange={(event) => setForm({ ...form, total_rooms: Number(event.target.value) })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
            </div>
            <textarea value={form.description ?? ''} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Description" className="min-h-20 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
            <input value={amenities} onChange={(event) => setAmenities(event.target.value)} placeholder="Amenities, comma separated" className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
