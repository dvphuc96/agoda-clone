    ...
    <nav className="sticky top-0 z-50 border-b border-border/70 bg-[#fffaf2]/90 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-8">
        <Link to="/" className="flex shrink-0 items-center gap-2 text-xl font-semibold tracking-tight text-navy">
          <span className="grid size-9 place-items-center rounded-md bg-navy text-sm font-bold text-white">GS</span>
        ...
        <button
          type="button"
          className="grid size-10 place-items-center rounded-md border border-border bg-white text-navy lg:hidden"
      ...
      {menuOpen && (
        <div id="mobile-menu" className="border-t border-border bg-[#fffaf2] p-4 text-sm shadow-lg lg:hidden">
          <div className="mx-auto flex max-w-7xl flex-col gap-3">
            <Link to="/search" className="rounded-md p-2 font-medium text-text hover:bg-tab" onClick={() => setMenuOpen(false)}>{t('nav.hotels')}</Link>
            <a href="#destinations" className="rounded-md p-2 font-medium text-text hover:bg-tab" onClick={() => setMenuOpen(false)}>{t('nav.destinations')}</a>
            <a href="#featured" className="rounded-md p-2 font-medium text-text hover:bg-tab" onClick={() => setMenuOpen(false)}>{t('nav.deals')}</a>
