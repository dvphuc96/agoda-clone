...
export default function HotelDetailPage() {
  ...
  if (isError || !hotel) {
    return (
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-16 text-center" role="alert" aria-label={t('common.error')}>
        <div className="text-5xl mb-4">😕</div>
        <h2 className="text-xl font-bold text-text">{t('hotel.notFoundTitle')}</h2>
        <p className="text-sm text-text-secondary mt-2">{t('hotel.notFoundBody')}</p>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 space-y-8">
      ...
      <div className="flex items-center gap-2 text-sm text-text-secondary">
        ...
        <span>/</span>
        <Link to="/search" className="hover:text-primary transition-colors">{t('common.search')}</Link>
        <span>/</span>
      ...
      {/* Room Types */}
      <div>
        <h2 className="text-xl font-bold text-text tracking-tight mb-4">{t('hotel.rooms')}</h2>
